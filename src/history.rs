...
pub async fn get_history(contract_address:&str,chain_name:String,start_block:u64,end_block:u64)-> eyre::Result<()>{
...
//chain was to be generalized
    let client = Client::builder().with_api_key(api_key).chain(Chain::Mainnet).unwrap().build().unwrap();
...
//Fetching all transactions
        let txns=client.get_transactions(&contract_address.parse().unwrap(),Some(paras)).await.unwrap();
        // println!("{:?}",&txns);
