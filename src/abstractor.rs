...
pub async fn create_meta(meta_slug: &str, event_doc:IndexedTransaction) {
    ...
    if meta_schema.is_empty() == true {
        ...
        for element in loaded_meta_schema.source {
            ...
            // let mut logs_data = match db::get_logs_data(
            //     &contract,
            //     &method,
            //     &action_type,
            //     block_number,
            ...
            // )
            // .await
            // let logs_data=event_doc;
            // {
            //     Some(logs) => {
            //         // println!("the size of logs is {:?}", logs.);
            //         logs
            //     },
            //     None => {
            //         error!("Didn't find any transaction logs \nRequires Manual Intervention at this point\n\n");
            ...
            // transactions or event loop
            // while let Some(doc) = logs_data.next().await {
            ...
            println!("this is the txn while loop starting");
                ...
                for param in loaded_transaction.transaction.method_params {
                    if prop_list.contains(&param.name) {
                            ...
                            match response {
                                Ok(object) => {
                                    if object.status().is_success() {
                                        let res =
                                            object.text().await.expect("Error in parsing object");
                        ...
                        // erc721 module
                            ...
                            info!("Found a match of erc721 module {:?} {:?}\n", param.name, param_value);
                ...
                info!("The meta raw list element -------------> {:?}\n", metadata_list);
