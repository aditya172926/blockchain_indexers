...
pub fn utils_network_data(chain_id: u64) -> Option<NetworkStruct> {
    ...
    let network_rpc = match network_details {
        Ok(object) => {
            let mut network_name: String = object["production"][chain_id.to_string()]["network_name"].to_string();
            let mut network_rpc_url: String = object["production"][chain_id.to_string()]["network_rpc_url"].to_string();
            let mut network_api_key: String = object["production"][chain_id.to_string()]["network_api_key"].to_string();
            let start_block_number: u64 = object["production"][chain_id.to_string()]["start_block_number"]
                .to_string()
                .parse()
                .unwrap();
