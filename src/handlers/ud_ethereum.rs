...
pub async fn handler_ud(
        ...
        || transaction_indexed.method.name == "safeTransferFrom"
    {
        ...
        let result: MetaResult = MetaResult {
            ...
            owner: transaction_indexed.method.params[0].to_string(),
        ...
        return Some(result);
    } else if transaction_indexed.method.name == "setOwner" {
        let update_obj: HashMap<String, String> = HashMap::from([
            (
                String::from("document.owner"),
                format!("0x{}", transaction_indexed.method.params[0]),
            ),
            (
        ...
        let result = MetaResult {
            id: transaction_indexed.method.params[1].to_string(),
            owner: format!("0x{}", transaction_indexed.method.params[0]),
            slug: schema.slug.clone(),
            insert: None,
