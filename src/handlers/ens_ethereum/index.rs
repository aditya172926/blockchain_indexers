...
pub async fn handler_ens(
    ...
    match &transaction_indexed.method{
        Some(transaction_indexed_method)=>{
            if transaction_indexed_method.name == "register" || transaction_indexed_method.name == "registerOnly" {
                return handler::handler_txn_register_ens(transaction_indexed,schema).await;
            } else if transaction_indexed_method.name == "renew" {
                return handler::handler_txn_renew_ens(transaction_indexed,transaction_indexed_method,schema).await;
            } else if transaction_indexed_method.name == "reclaim" {
                return handler::handler_txn_reclaim_ens(transaction_indexed,transaction_indexed_method,schema).await;
            } else {
                return None;
            }
            },
        ...
        None => {
            ...
            if transaction_event.name.to_string()=="NameRegistered"{
                    return handler::handler_event_register_ens(transaction_indexed,schema).await;
                }else if transaction_event.name.to_string()=="NameRenewed"{
                    return handler::handler_event_renew_ens(transaction_indexed,schema).await;
                }else if transaction_event.name.to_string()=="Transfer"{
                    return handler::handler_event_transfer_ens(transaction_indexed,transaction_event.clone(),schema).await;
                }
                else{
                    return None;
                }
