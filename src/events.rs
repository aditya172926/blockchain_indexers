...
pub async fn get_history_events(
    ...
    let event_filter: Filter = Filter::new()
        ...
        .topic0(ValueOrArray::Array(contract_metadata.events_of_interest.topics.clone()));

    let logs = client.get_logs(&event_filter).await.unwrap();
    for log in logs {
        info!(
            "\n\nTransaction hash -> {:?} \n\nLog topics -> {:?} \n Log data -> {:?}\n\n",
            log.transaction_hash, log.topics[0], log.data
        );
        match contract_instance.decode_event_raw(contract_metadata.events_of_interest.map[log.topics[0]], log.topics, log.data) {
            Ok(inputs) => {
